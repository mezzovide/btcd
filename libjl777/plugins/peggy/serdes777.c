...
#define PRICE_RESOLUTION_MAXPVAL ((int64_t)3037000500)  // 303.5613528178975 vs 64 bits: 4294967295  429.30058206405493,
#define PRICE_RESOLUTION_MAXUNITS ((int16_t)((int64_t)0x7fffffffffffffffLL / (SATOSHIDEN * PRICE_RESOLUTION)))  // 9219
...
struct opreturn_buffers { int32_t blocktimestamp,blocknum,num; struct opreturn_entry *ptrs[]; };
...
int32_t serdes777_deserialize(int32_t *signedcountp,struct peggy_tx *Ptx,uint32_t blocktimestamp,uint8_t *data,int32_t totallen)
{
    ...
    {
        printf("serdes777_process error totalbits.%d remains.%d nonzero modval.%ld\n",totalbits,remains,(remains % sizeof(bits256)));
        return(-1);
    }
